  ...
  break
			...
			if (!isUser) return reply(mess.only.userB)
				...
				case 'daftar':
					...
					captionnya = `╭─「 *_𝙿𝚎𝚛𝚏𝚒𝚕 𝚍𝚎 𝚞𝚗 𝚖𝚖𝚐_* 」\`\`\`\n│ 𝙸𝙽𝙳𝙴𝙽𝚃𝙸𝙵𝙸𝙲𝙰𝙲𝙸𝙾𝙽 : \n│TM08GK8PPHBSJDH10J\`\`\`\n│\n│\`\`\`𝙳𝙸𝙰 ${date} ${time}\`\`\`\n│\`\`\`「 𝙽𝙾𝙼𝙱𝚁𝙴」: ${pushname2}\`\`\`\n│\`\`\`「 𝙽𝚄𝙼𝙴𝚁𝙾 」: wa.me/${sender.split("@")[0]}\`\`\`\n│\`\`\`𝚄𝚂𝙰𝚁 𝙴𝙻 𝙱𝙾𝚃 𝙳𝙴 𝙹𝚄𝚂𝚃𝙸𝙽\`\`\`\n│\`\`\`𝚁𝙰𝙿𝙰 𝚃𝚄 𝙼𝙰𝙳𝚁𝙴\`\`\`\n│\`\`\`𝚈𝙰 𝙿𝚄𝙴𝙳𝙴𝚂 𝙴𝚂𝙲𝚁𝙸𝙱𝙸𝚁 ${prefix}menu\`\`\`\n│\`\`\`\n│𝚀𝚄𝙴 𝚃𝙰𝙽 𝙼𝙼𝙶 𝙴𝚁𝙴𝚂: ${user.length} 𝚂𝙸 𝙴𝚂 𝙼𝙼𝙶\`\`\`\n╰────────────────`
					brando = await getBuffer(`https://pencarikode.xyz/api/cita-cita?apikey=pais`, {method: 'get'})                
...
// only grup fitur anime
              case 'anime':
                ...
                if (isLimit(sender)) return reply(limitend(pushname2))
                if (!isAnime) return reply(' *Sabes yo fui igual que tu una vez* ')
...
quoted: mek, caption: `Hasil Dari *${daerah}*`
  ...
  break
				...
				if (!isGroup) return reply(mess.only.group)
				if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('BIEVENIDO SINGA TU MADRE')
					if ((args[0]) === 'on') {
...
case 'husbu':
		...
		denz.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/@rulay.mp3')
        denz.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        const d = fs.readFileSync('./sticker/@rulay.webp');
